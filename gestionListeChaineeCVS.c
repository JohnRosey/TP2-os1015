...
//#######################################
//#
//# Affiche le message passé en argument dans la fifo du
//# 

void writeOnClientFifo(int pid_client, char message[400]){
	int client_fifo_fd;
	char client_fifo[256];
	struct Info_FIFO_Transaction reponseAuClient;

	// création de la fifo client
	sprintf(client_fifo, CLIENT_FIFO_NAME, pid_client);

	//ouverture de la fifo client
    client_fifo_fd = open(client_fifo, O_WRONLY);

    if (client_fifo_fd != -1) {
        reponseAuClient.pid_client = pid_client;
        sprintf(reponseAuClient.transaction,"%s",message);
        write(client_fifo_fd, &reponseAuClient, sizeof(reponseAuClient));
...
void listItems(struct infoLIST* param){ 
	...
	// on répond directement dans le fifo du client
	writeOnClientFifo(pid_client,"noligne  texte                 \n");
	writeOnClientFifo(pid_client,"======= =======================\n");
	...
	while (ptr!=NULL){
		...
		if ((ptr->ligne.noligne>=start)&&(ptr->ligne.noligne<=end)){
			sprintf(message,"%d \t %s\n",ptr->ligne.noligne,ptr->ligne.ptrligne);
			writeOnClientFifo(pid_client,message);
			}
